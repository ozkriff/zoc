...
pub trait CheckCommand {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult;
}

impl CheckCommand for command::EndTurn {
    fn check(&self, _: &Db, _: PlayerId, _: &State) -> CheckResult {
        Ok(())
    }
}

impl CheckCommand for command::CreateUnit {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
...
impl CheckCommand for command::Move {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
...
impl CheckCommand for command::AttackUnit {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
...
impl CheckCommand for command::LoadUnit {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
...
impl CheckCommand for command::UnloadUnit {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
        let transporter = match state.unit_opt(self.transporter_id) {
            Some(transporter) => transporter,
            None => return Err(CommandError::BadTransporterId),
...
impl CheckCommand for command::Attach {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
        let transporter = match state.unit_opt(self.transporter_id) {
            Some(transporter) => transporter,
            None => return Err(CommandError::BadTransporterId),
...
impl CheckCommand for command::Detach {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
        let transporter = match state.unit_opt(self.transporter_id) {
            Some(transporter) => transporter,
            None => return Err(CommandError::BadTransporterId),
...
impl CheckCommand for command::SetReactionFireMode {
    fn check(&self, _: &Db, player_id: PlayerId, state: &State) -> CheckResult {
        let unit = match state.unit_opt(self.unit_id) {
            Some(unit) => unit,
            None => return Err(CommandError::BadUnitId),
        };
...
impl CheckCommand for command::Smoke {
    fn check(&self, db: &Db, player_id: PlayerId, state: &State) -> CheckResult {
        let unit = match state.unit_opt(self.unit_id) {
            Some(unit) => unit,
            None => return Err(CommandError::BadUnitId),
